...
def load_model_s3(model_name):
    """Charge le modèle depuis models/ (local, déjà présent dans le repo) — bascule sur S3 si absent et configuré."""
    ...
    raise FileNotFoundError(
        f"Modèle introuvable : ni {local_path}, ni de bucket S3 configuré."
    )

@st.cache_data
def load_data():
    """Charge les données traitées en local (data/processed/) — bascule sur S3 si configuré et absent en local."""
    local_dir = "data/processed"
    if os.path.exists(f"{local_dir}/team_stats_raw.csv"):
        team_stats = pd.read_csv(f"{local_dir}/team_stats_raw.csv")
        features   = pd.read_csv(f"{local_dir}/dataset_features.csv")
    ...
    raise FileNotFoundError(
        "Données introuvables : lancez d'abord les notebooks 01 et 02, "
        "ou configurez un bucket S3 dans .env."
    )
